...
SUBROUTINE laxlib_cdiaghg_omp( n, m, h, s, ldh, e, v, me_bgrp, root_bgrp, intra_bgrp_comm )
  ...
  !
  ! G.R. - This version allocates dummy arrays and buffers on the CPU and then maps everything
  !        on the GPU: this is a sub-optimal and non-performant solution, waiting for the next
  !        release of devXlib coming into QE
  !
  USE laxlib_parallel_include
  !
  USE onemkl_lapack_omp_offload
!This part is not working yet, just allocating data on the host and mapping it
...
!#endif
  ...
  COMPLEX(DP), ALLOCATABLE :: h_bkp(:,:), s_bkp(:,:)
  character*1 :: jobz = 'V'
  ...
  !
  !
  CALL start_clock_gpu( 'cdiaghg_omp' )
  !
...
!#endif
      allocate(ifail(n), h_bkp(n,n), s_bkp(n,n), rwork(7*n), iwork(5*n))
      !$omp target data map(alloc:ifail, h_bkp, s_bkp, rwork, iwork)
      !
      !$omp target teams distribute parallel do collapse(2) map(present,alloc:h_bkp,s_bkp)
      DO j=1,n
         DO i=1,n
            h_bkp(i,j) = h(i,j)
            s_bkp(i,j) = s(i,j)
...
!#endif
     allocate(work(lwork))
     !$omp target data map(alloc:work)
...
!      deallocate(ifail, h_bkp, s_bkp, work, rwork, iwork)
!#else
...
#endif
  !
  CALL stop_clock_gpu( 'cdiaghg_omp' )
  !
  RETURN
  !
...
SUBROUTINE laxlib_cdiaghg( n, m, h, s, ldh, e, v, me_bgrp, root_bgrp, intra_bgrp_comm )
  ...
  USE laxlib_parallel_include
#if defined(__OPENMP_GPU)
  USE LAXlib, ONLY : laxlib_cdiaghg_omp
#endif
  IMPLICIT NONE
  include 'laxlib_kinds.fh'
  !
