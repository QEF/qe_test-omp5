...
!#endif
  ...
  REAL(DP), ALLOCATABLE :: h_bkp(:,:), s_bkp(:,:)
  character*1 :: jobz = 'V'
...
!     allocate(ifail(n), h_bkp(n,n), s_bkp(n,n), iwork(3+5*n))
!#else
...
!#endif
      allocate(ifail(n), h_bkp(n,n), s_bkp(n,n), iwork(3+5*n))
      !$omp target data map(alloc:ifail, h_bkp, s_bkp, iwork)
     !
...
!     deallocate(ifail, h_bkp, s_bkp, work, iwork)
!#else
...
SUBROUTINE laxlib_rdiaghg( n, m, h, s, ldh, e, v, me_bgrp, root_bgrp, intra_bgrp_comm )
  ...
  USE laxlib_parallel_include
#if defined(__OPENMP_GPU)
  USE LAXlib, ONLY : laxlib_rdiaghg_omp
#endif
  !
  IMPLICIT NONE
  include 'laxlib_kinds.fh'
